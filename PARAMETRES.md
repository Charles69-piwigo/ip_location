...
| `ip_location_score_multi_url_burst` | `50` | Points ajoutés en cas de **rafale multi-URL mono-IP** : ≥ 10 URL distinctes accédées par la même IP dans une fenêtre d'environ 30 secondes (v2.5) — typique d'un scraper qui parcourt le catalogue à grande vitesse, à la différence de la rafale mono-IP ci-dessus qui répète une même URL. |
...
- Ces paramètres ne concernent que le **score bot** (4ème levier). Les autres
  réglages du plugin (pays bloqués, whitelist IP, seuil de score, mode de
  blocage, filtre pays téléchargement, etc.) sont gérés via l'onglet
...
- Le blocage automatique par score ne s'active que si `bot_block_enabled` **et**
  `htaccess_enabled` sont actifs dans la config du plugin (réglages admin), en
  plus de ces paramètres fins.
