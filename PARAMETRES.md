...
| `ip_location_refusal_log_minutes` | `10` | (v2.6.5) Un refus répété — même IP, même motif (robot bloqué, liste d'IP, blocage automatique) — n'est journalisé qu'une fois par période de N minutes : la requête reste refusée à chaque fois, mais un robot qui insiste (GPTBot observé à ~50 requêtes/minute pendant des heures) ne remplit plus le journal. `0` = journaliser chaque refus. |
| `ip_location_classify_interval_hours` | `4` | Intervalle minimal (en heures) entre deux déclenchements de la classification/blocage auto via le **trafic public** (`ip_location_log_visit()`), pour que le mécanisme fonctionne même sans visite admin régulière. Sans effet sur le déclenchement depuis l'onglet admin (voir ligne suivante). |
| `ip_location_classify_admin_interval_minutes` | `10` | (v2.7a.4) Intervalle minimal (en minutes) entre deux passages de la classification/blocage auto déclenchés par l'ouverture de l'onglet admin du plugin. Avant, le calcul était relancé à chaque chargement, ce qui faisait attendre de longues secondes sur un site très exploré (journal plein de passages de Googlebot). `0` = à chaque chargement (ancien comportement). L'enregistrement du bloc « Blocage automatique » force toujours un recalcul immédiat. |
| `ip_location_robot_log_limit` | `100` | (v2.7a.5) Nombre d'accès **journalisés** par robot d'indexation autorisé et vérifiable par DNS (Googlebot, Bingbot, Applebot, Yandex, Baidu) et par jour. Au-delà, les accès ne sont plus enregistrés dans le journal mais seulement comptés (table `ip_location_robot_count`, par jour / robot / IP / pays) ; les compteurs du Journal et les séries « Tous » / « Bots » des statistiques les incluent. `0` = tout journaliser (ancien comportement). Les robots autorisés sans vérification DNS, les faux robots et les robots bloqués sont toujours journalisés. |
